...
def profile():
    ...
    artist = Artist.query.filter_by(ArtistId=user_id).first()
    if artist:
        return redirect(url_for('artist_details', artist_id=user_id))

    # Check if the user is a visitor
    visitor = Visitor.query.filter_by(VisitorID=user_id).first()
    if visitor:
        purchased_artworks = Artwork.query.filter_by(VisitorID=visitor.VisitorID).all()
        return render_template('profile_visitor.html', user=user, visitor=visitor, purchased_artworks=purchased_artworks)

    return "User type not recognized", 404
